$(document).ready(function (){
    ...
    // Send salary request
    $(document).on('click', '.requestSalary', function () {
        ...
        var a_acct_num = $('#a_acct_num').val();
        var a_acct_name = $('#a_acct_name').val();
        var a_bank = $('#a_bank').val();
        var a_worktime = $('#a_worktime').val();
        var a_request_amount = $('#a_request_amount').val();

        if(a_name != '' && a_email != '' && a_phone != '' && a_acct_num != '' && a_acct_name != '' && a_bank != '' && a_worktime != '' && a_request_amount != '')
        {
            var data = {
                'sendRequestBtn': true,
                'admin_id': admin_id,
                ...
                'a_email': a_email,
                'a_phone': a_phone,
                'a_acct_num': a_acct_num,
                'a_acct_name': a_acct_name,
                'a_bank': a_bank,
                'a_worktime': a_worktime,
                'a_request_amount': a_request_amount,
            }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(res.status == 200){
                        swal(res.message, res.message, res.status_type);
                        $('#requestSalaryModal').modal('.close');
                    }else if(res.status == 422){
                        swal(res.message, res.message, res.status_type);
                    }else{
                        swal(res.message, res.message, res.status_type);
