...
// Sending Mail
function sendemail_order($fullName,$customerEmail,$sessionProducts,$grandTotal,$trans_ref,$payment_mode)
{
    ...
    $email_template = '
        <div>
            ...
            <p>Your Order Was successful. Your order id is ' . htmlspecialchars($trans_ref) . '.</p>
...
if(isset($_GET["transaction_id"]) AND isset($_GET["status"])  AND isset($_GET["tx_ref"])){
    ...
    if(($status != $trans_status) || ($trans_id != $id) || ($charged_amount != $amount) || ($currencyPaid != $currency)){
        ...
        sendemail_order($fullName,$customerEmail,$sessionProducts,$grandTotal,$trans_ref,$payment_mode);
        ...
        header("Location: paid.php?transaction_id=$id&status=$status&tx_ref=$reference&name=$fullName");
    }
    curl_close($curl);
