...
<nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" data-scroll="false" style="background: #6f42c1; border-radius: 15px;">
    <div class="container-fluid py-1 px-3">
        ...
        <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-white active" aria-current="page">Today Orders</li>
        </ol>
        <h6 class="font-weight-bolder text-white mb-0">Today Orders</h6>
        </nav>
    </div>
</nav>
<!-- End Navbar -->
...
<div class="container-fluid px-4">
    <div class="card mt-4 shadow-sm">
        <div class="card-header">
            <div class="row">
                <div class="col-md-4">
                    <h5 class="m-0 font-weight-bold" style="color: #6f42c1">Today Orders</h5>
                </div>
                <div class="col-md-8">
                    <form action="" method="GET">
                        <div class="row g-1">
                            ...
                            <div class="col-md-4">
                                <button type="submit" class="btn text-white" style="background: #6f42c1;">Filter</button>
                                <a href="today-orders.php" class="btn btn-danger">Reset</a>
                            </div>
                        </div>
                    </form>
                </div>
            ...
            <?php 
                ...
                if(isset($_GET['date']) || isset($_GET['payment_status'])){
                    ...
                    if($orderDate != '' && $paymentStatus == ''){
                        ...
                        WHERE c.id = o.customer_id AND o.order_date='$todayDate' ORDER BY orders_id DESC";
                    }elseif($orderDate == '' && $paymentStatus != ''){
                        $query = "SELECT o.id as orders_id, o.*, c.* FROM orders o, customers c
                        WHERE c.id = o.customer_id AND o.payment_mode='$paymentStatus' AND o.order_date='$todayDate' ORDER BY orders_id DESC";
                    }elseif($orderDate != '' && $paymentStatus != ''){
                        $query = "SELECT o.id as orders_id, o.*, c.* FROM orders o, customers c
                        WHERE c.id = o.customer_id 
                        AND o.order_date='$orderDate' 
                        AND o.payment_mode='$paymentStatus'
                        AND o.order_date='$todayDate' ORDER BY orders_id DESC";
                    }else{
                        $query = "SELECT o.id as orders_id, o.*, c.* FROM orders o, customers c
                        WHERE c.id = o.customer_id AND o.order_date='$todayDate' ORDER BY orders_id DESC";
                    }
                                ...
                                <tbody>
                                    <?php foreach($orders as $orderItem) :?>
                                        <tr>
                                            ...
                                            <td>
                                                <a href="today-orders-view.php?track=<?= $orderItem['tracking_no']; ?>" class="btn btn-info mb-0 px-2 btn-sm">View</a>
